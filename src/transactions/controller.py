...
async def create_entity(
    ...
    try:
        ...
        entity_model = TransactionModel(**entity_out.model_dump())
...
async def get_all_entities(
    ...
    if name:
        query = query.filter(TransactionModel.name.ilike(f'%{name}%'))
    
    if is_active is not None:
        query = query.filter(TransactionModel.is_active == is_active)
...
async def get_entity_by_id(
    ...
    result = await db_session.execute(
        select(TransactionModel).filter_by(id=entity_id)
    )
...
#     query = select(TransactionModel).filter(
#         or_(
#             TransactionModel.name.ilike(f'%{q}%'),
